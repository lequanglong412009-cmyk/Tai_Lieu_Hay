  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[60] py-2 md:py-4 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto h-16 md:h-20 px-4 md:px-6 rounded-[1rem] md:rounded-[1.5rem] bg-white/70 backdrop-blur-xl border border-white shadow-[0_8px_32px_rgba(0,0,0,0.04)] flex items-center justify-between transition-all duration-500">
        <Link 
          to={user?.email === 'tailieuhay53@gmail.com' ? (location.pathname === '/admin' ? "/" : "/admin") : ((location.pathname === '/profile' && (!location.search || location.search.includes('tab=purchased'))) ? "/" : "/profile?tab=purchased")} 
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            ) : (location.pathname === '/profile' && (!location.search || location.search.includes('tab=purchased'))) ? (
              <span className="text-[10px] md:text-xs text-slate-500 font-medium mt-0.5 group-hover:text-indigo-600 transition-colors">
                nhấp vào đây quay trở về trang chủ
              </span>
            ) : (
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-4">
              <Link to={user?.email === 'tailieuhay53@gmail.com' ? '/admin' : '/profile'} className="flex items-center gap-3 p-1.5 pr-4 rounded-full bg-slate-50 border border-slate-100 hover:bg-white hover:border-indigo-100 transition-all group shadow-sm hover:shadow-md">
                ...
                <div className="hidden sm:block text-left">
                  <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest leading-none mb-0.5">{user.username || user.displayName || 'Học sinh'}</p>
                  <p className="text-[9px] text-emerald-600 font-bold">{user.isAdmin ? 'Quản trị viên' : 'Học sinh'}</p>
                </div>
              </Link>
              <button 
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Đăng nhập</h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Truy cập kho tài liệu của bạn</p>
                </div>
                <button 
                  onClick={() => setIsLoginModalOpen(false)}
              ...
              <div className="space-y-4">
                ...
                {error && (
                  <p className="text-xs text-red-500 font-bold text-center bg-red-50 py-3 rounded-xl border border-red-100 px-4">{error}</p>
                )}
                ...
                <p className="text-[9px] text-[#6B7280] font-bold text-center px-2 leading-relaxed">
                  Đăng nhập bằng Gmail giúp bạn quản lý tài liệu và khóa học đã mua một cách an toàn nhất.
                </p>
              </div>
            </motion.div>
          </div>
